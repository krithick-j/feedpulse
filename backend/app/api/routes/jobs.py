...
from app.api.handlers.jobs import job_handler
...
async def list_jobs() -> List[JobSummary]:
    return await job_handler.list_jobs()
...
async def start_job(payload: Optional[StartJobRequest] = None) -> StartJobResponse:
    return await job_handler.start_job(payload)


@router.get("/jobs/{job_id}", response_model=JobDetail)
async def get_job(job_id: str) -> JobDetail:
    return await job_handler.get_job(job_id)
...
async def get_task_records(
    ...
    return await job_handler.get_task_records(job_id, task_id, limit=limit, offset=offset)


@router.get("/jobs/{job_id}/events")
async def stream_job_events(job_id: str) -> StreamingResponse:
    return await job_handler.stream_job_events(job_id)
